...
export default function BrewSimulator({ lang, challenge }) {
  const [grind, setGrind] = useState(8);  // coarse → under-extracted by default
  const [ratio, setRatio] = useState(18); // more water → weaker
  const [temp, setTemp] = useState(88);   // cooler → less extraction
  const [agitation, setAgitation] = useState(2);
  ...
  const tr = {
    en: {
      ...
      ratio: "Brew Ratio (1:X)", temp: "Water Temp (°C)",
      agitation: "Agitation / Pours", low: "Low", high: "High",
      ey: "Extraction Yield (%)", tds: "TDS / Strength (%)",
      ideal: "Ideal Balance", sour: "Sour / Under-extracted",
      bitter: "Bitter / Over-extracted", strong: "Strong", weak: "Weak",
      startChallenge: "Start Challenge", freePlay: "Free play",
      tryAgain: "Try Again", skip: "Skip / Free play",
      inZone: "✅ In the Gold Cup zone!",
      notYet: "Not in zone yet",
    },
    es: {
      grind: "Molienda", fine: "Fino", coarse: "Grueso",
      ratio: "Proporción (1:X)", temp: "Temp. del Agua (°C)",
      agitation: "Agitación / Vertidos", low: "Baja", high: "Alta",
      ey: "Rendimiento de Extracción (%)", tds: "TDS / Fuerza (%)",
      ideal: "Equilibrio Ideal", sour: "Ácido / Subextraído",
      bitter: "Amargo / Sobreextraído", strong: "Fuerte", weak: "Débil",
      startChallenge: "Iniciar Desafío", freePlay: "Modo libre",
      tryAgain: "Intentar de nuevo", skip: "Saltar / Modo libre",
      inZone: "✅ ¡En la zona Gold Cup!",
      notYet: "Aún fuera de la zona",
    }
  }[lang];

  let status = tr.ideal;
  if (ey < 18) status = tr.sour;
  else if (ey > 22) status = tr.bitter;
  else if (tds > 1.45) status = tr.strong;
  else if (tds < 1.15) status = tr.weak;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ background: 'var(--bg)', border: '2px solid var(--accent)', borderRadius: '8px', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '13px', marginBottom: '8px' }}>
            ...
            <span style={{ color: tds >= challenge.target.tds[0] && tds <= challenge.target.tds[1] ? 'var(--done)' : '#c0392b' }}>
              TDS: {tds.toFixed(2)}% (target {challenge.target.tds[0]}–{challenge.target.tds[1]}%)
            </span>
          </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', background: 'var(--bg)', padding: '16px', borderRadius: '8px' }}>
        ...
        <div style={{ flex: '1 1 200px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 'bold' }}>
            <label>{tr.agitation}</label>
            <span>{agitation <= 2 ? tr.low : agitation >= 4 ? tr.high : 'Med'}</span>
          </div>
      ...
      <div style={{ width: '100%', height: '350px' }}>
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <YAxis type="number" dataKey="y" name={tr.tds} unit="%" domain={[0.8, 1.8]} tickCount={6} />
